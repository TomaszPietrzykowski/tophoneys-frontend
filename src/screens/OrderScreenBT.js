...
const OrderScreenBT = ({ match, history }) => {
  ...
  const renderButtons = () => {
    ...
    window.paypal
      .Buttons({
        ...
        client: {
          sandbox: window.paypal.request.get(
            "/api/braintree/token?env=sandbox"
          ),
          production: window.paypal.request.get(
            "/api/braintree/token?env=production"
          ),
        },
        // Set up the payment:
        ...
        onAuthorize: function (data, actions) {
          ...
          return actions.payment.tokenize().then(function (data) {
            ...
            return window.paypal.request.post("/api/braintree/execute", {
              nonce: data.nonce,
            })
          })
        },
  ...
  useEffect(() => {
    const addPayPalScript = async () => {
      ...
      bt.src = `https://js.braintreegateway.com/web/3.79.1/js/client.min.js`
      bt.async = true
    ...
    <>
      ...
      <div className={classes.container}>
        {loading ? (
          ...
          <>
            <h1 className={classes.title}>Order details (braintree)</h1>
            <Grid container>
